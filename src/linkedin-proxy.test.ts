...
function makeTestDb(): InstanceType<typeof Database> {
  ...
  db.exec(`CREATE TABLE linkedin_calls (id INTEGER PRIMARY KEY AUTOINCREMENT, called_at TEXT NOT NULL);`);
  return db;
}
...
describe('createLinkedInProxy', () => {
  ...
  it('rejects send_message tool calls', async () => {
    const res = await postJson(PORT, {
      ...
      params: { name: 'send_message', arguments: { recipient: 'x', message: 'y' } },
  ...
  it('rejects calls when rate limit is exceeded', async () => {
    ...
    const localProxy = createLinkedInProxy({ limiter, upstreamPort: 19999, port: 18083 });
  ...
  it('returns 404 for non-MCP paths', async () => {
    const res = await new Promise<number>((resolve, reject) => {
      const req = http.request({ hostname: 'localhost', port: PORT, path: '/other', method: 'GET' }, (r) => resolve(r.statusCode ?? 0));
      req.on('error', reject);
      req.end();
    });
    expect(res).toBe(404);
